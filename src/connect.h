...
// #include <unistd.h>
template<typename T>
void sendInt64(const int64_t &data, size_t player)
{
	vector<int64_t> vec = {data};
	if(!communicationSenders[player]->sendMsg(vec.data(), sizeof(T), 0))
		cout << "Send vector error" << endl;
}
template<typename T>
void receiveInt64(int64_t &data, size_t player)
{
	vector<int64_t> vec(1, 0);
	if(!communicationReceivers[player]->receiveMsg(vec.data(), sizeof(T), 0))
		cout << "Receive myType vector error" << endl;
	data = vec[0];
}

template<typename T>
void sendMyType(const T &data, size_t player)
{
	vector<T> vec = {data};
	if(!communicationSenders[player]->sendMsg(vec.data(), sizeof(T), 0))
		cout << "Send vector error" << endl;
...
template<typename T>
void receiveMyType(T &data, size_t player)
{
	vector<T> vec(1, 0);
	if(!communicationReceivers[player]->receiveMsg(vec.data(), sizeof(T), 0))
		cout << "Receive myType vector error" << endl;
