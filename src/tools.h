...
{	
	...
	if (r_type == "r_1")
	{
		assert((partyNum == PARTY_C) && "Only C can call for bits");
		assert(neg_type == "POSITIVE" && "neg_type should be POSITIVE");
		assert(sizeof(T) == sizeof(smallType) && "sizeof(T) == sizeof(myType)");
	...
	if (r_type == "s_1")
	{
		assert((partyNum == PARTY_C) && "Only C can call for bits");
		assert(neg_type == "POSITIVE" && "neg_type should be POSITIVE");
		assert(sizeof(T) == sizeof(int64_t) && "sizeof(T) == sizeof(myType)");
		
		//srand((unsigned)time(NULL));
		for (size_t i = 0; i < size; ++i)
			vec[i] = (rand()%((1 <<LENGTH)-1));
		//cout<<"-------------------------test------------------------"<<endl;
	}
	if (r_type == "v_1")
	{
		assert((partyNum == PARTY_A) && "Only A can call for bits");
		assert(neg_type == "POSITIVE" && "neg_type should be POSITIVE");
		assert(sizeof(T) == sizeof(smallType) && "sizeof(T) == sizeof(myType)");
		//srand((unsigned)time(NULL));
		for (size_t i = 0; i < size; ++i)
...
template<typename T>
void xorVectors(const vector<T> &a, const vector<T> &b, vector<T> &c, size_t size)
{
	for (size_t i = 0; i < size; ++i)
		c[i] = a[i] ^ b[i];
}
...
void binary(int64_t a, vector<smallType>& v, size_t size);
void printVector(const vector<smallType>& v);
bool func(smallType a);
void inverseCode(vector<smallType>& v);
void complement(vector<smallType>& v, size_t size);
